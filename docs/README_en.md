...
│   │   ├── EffectMeta.cs.in       # Template for effect metadata classes
...
`category` and `filterName` are optional. Defaults: `"Bridge Effects"` / the effect ID.
...
1. Reads `id`, `category`, and `filterName` from `effects.json` (using `jq`)
2. Generates `{id}Meta.cs` (category/filter name constants class) from `EffectMeta.cs.in` for every effect
3. Generates `{id}.cs` from `EffectTemplate.cs.in` for effects without `"custom": true` (output to `build/CSPBridgeEffects/`)
4. Builds `{id}.cpm` (C++ bridge DLL)
5. Builds `CSPBridgeEffects.dll` (including generated `.cs` files)
...
Meson auto-generates two files per effect from `effects.json`.

**`{id}Meta.cs`** (every effect, from [`EffectMeta.cs.in`](../CSPBridgeEffects/Effects/EffectMeta.cs.in))

A constants class holding the category name and filter name. Referenced by both generated and custom implementations.

| Placeholder | Replaced with | Example |
| --- | --- | --- |
| `@EFFECT_ID@` | `id` in effects.json | `Blur` |
| `@CATEGORY@` | `category` in effects.json (default: `"Bridge Effects"`) | `"My Filters"` |
| `@FILTER_NAME@` | `filterName` in effects.json (default: effect ID) | `"Blur Effect"` |

**`{id}.cs`** (effects without `"custom": true`, from [`EffectTemplate.cs.in`](../CSPBridgeEffects/Effects/EffectTemplate.cs.in))

Entry-point skeleton class. References `{id}Meta.Category` / `{id}Meta.FilterName` internally.
